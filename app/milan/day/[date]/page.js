...
const STATION_ICAO = "LIMC";
const STATION_NAME = "Milan Malpensa";
...
function formatMilanDateTimeSeconds(epochMs) {
...
function formatMilanClock(epochMs) {
  if (!Number.isFinite(epochMs)) {
    return "—";
  }
  const formatter = new Intl.DateTimeFormat("en-US", {
...
function formatOfficialPollMessage(result) {
  if (!result?.ok) {
    return "Latest MeteoAM poll skipped.";
  }

  const firstSeenText = Number.isFinite(result.row?.meteoAmFirstSeenAt)
    ? formatMilanDateTimeSeconds(result.row.meteoAmFirstSeenAt)
    : null;
  const lagText = Number.isFinite(result.availabilityLagMs)
    ? `${Math.max(0, result.availabilityLagMs / 60000).toFixed(1)} min lag`
    : null;

  return `Latest MeteoAM poll: ${result.insertedCount > 0 ? "saved" : "no new report"} ${result.row?.reportType ?? "message"} ${result.row?.obsTimeLocal ?? ""}.${firstSeenText ? ` First seen ${firstSeenText}${lagText ? ` (${lagText})` : ""}.` : ""}`;
}

function formatRefreshMessage(officialResult, noaaResult) {
  const officialText = formatOfficialPollMessage(officialResult);
  if (noaaResult?.ok) {
    return `${officialText} NOAA race sample refreshed.`;
  }
  return officialText;
}
...
export default function MilanDayPage() {
  ...
  const milanTodayDate = milanTodayKey();
  const isToday = isDateValid && date === milanTodayDate;
  const quickPreviousDates = useMemo(() => buildPreviousDateKeys(date, 2), [date]);

  const pollLatest = useAction("milan:pollLatestStationMetar");
  const pollLatestNoaa = useAction("milan:pollLatestNoaaPublishRace");

  const dayData = useQuery(
    "milan:getDayStationRows",
    isDateValid
      ? {
  ...
  const raceData = useQuery("milan:getRecentPublishRaceReports", {
    stationIcao: STATION_ICAO,
    limit: 12,
    routineOnly: true,
  });
  ...
  useEffect(() => {
    ...
    if (!isToday) {
      setLiveMessage(
        "Historical LIMC dates depend on previously captured live MeteoAM rows. No date-bounded official history backfill is wired yet.",
      );
      return;
    }
    ...
    async function bootstrap() {
      ...
      try {
        const [officialResult, noaaResult] = await Promise.allSettled([
        ...
        setLiveMessage(
          noaaResult.status === "fulfilled"
            ? `LIMC MeteoAM sync failed: ${officialMessage}. NOAA race sample refreshed.`
            : `LIMC MeteoAM sync failed: ${officialMessage}`,
        );
      } catch (error) {
        console.error(error);
        if (!cancelled) {
          const message = error instanceof Error ? error.message : String(error);
          setLiveMessage(`LIMC sync failed: ${message}`);
        }
      } finally {
  ...
  }, [date, isDateValid, isToday, pollLatest, pollLatestNoaa]);
  ...
  async function handleRefreshNow() {
    ...
    inFlightRef.current = true;
    try {
      const [officialResult, noaaResult] = await Promise.all([
        pollLatest({ stationIcao: STATION_ICAO }),
        pollLatestNoaa({ stationIcao: STATION_ICAO }),
      ]);
      setLiveMessage(formatRefreshMessage(officialResult, noaaResult));
    } catch (error) {
      console.error(error);
      const message = error instanceof Error ? error.message : String(error);
  ...
  function handleGoToDate(event) {
    ...
    router.push(`/milan/day/${inputDate}`);
  }
    ...
    () => ({
      ...
      scales: {
        x: {
          ...
          min: 0,
          max: 1439,
          title: { display: true, text: "Local Time (Europe/Rome)" },
          ticks: {
            stepSize: 60,
            callback(value) {
    ...
    return (
      <main className="min-h-screen px-4 py-8 md:px-8">
        <div className="mx-auto max-w-3xl rounded-3xl border border-red-200 bg-white p-6">
          <h1 className="text-2xl font-semibold text-red-800">Invalid Milan date</h1>
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="rounded-3xl border border-line/80 bg-panel/90 p-6 shadow-[0_18px_50px_rgba(37,35,27,0.08)]">
          ...
          <h1 className="mt-3 text-2xl font-semibold text-foreground">
            {STATION_NAME} METAR Logger
          </h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-black/65">
            Public Italian-origin LIMC METAR from MeteoAM&apos;s Deda endpoint,
            stored live and compared against NOAA `tgftp` in a publish-race table.
          </p>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            >
              Current Date {milanTodayDate}
            </Link>
            {quickPreviousDates.map((previousDate) => (
              <Link
                key={previousDate}
                href={`/milan/day/${previousDate}`}
          ...
          >
            <label className="text-sm font-medium text-black/70" htmlFor="milan-day-picker">
              Pick Date
            </label>
            <input
              id="milan-day-picker"
          ...
          <div className="mt-4 inline-flex flex-wrap items-center gap-2 rounded-2xl border border-emerald-200 bg-emerald-50/80 px-4 py-3 text-sm text-emerald-950">
            <span className="font-semibold uppercase tracking-[0.16em] text-emerald-800">
              Milan Time
            </span>
            <span className="font-medium">{formatMilanClock(clockNowMs)}</span>
          </div>

          <p className="mt-4 text-sm text-black/70">
            {liveMessage ||
              (isToday
                ? "Waiting for MeteoAM sync..."
                : "Historical LIMC dates depend on previously captured live official rows.")}
          </p>
        </header>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-3xl border border-line/70 bg-white/90 p-5 shadow-[0_12px_28px_rgba(37,35,27,0.06)]">
            ...
            <p className="mt-2 text-sm text-black/65">
              The public MeteoAM LIMC endpoint is polled every minute because its
              publication lag drifts well beyond the nominal half-hour routine
              boundaries.
            </p>
          </div>
          ...
          <div className="rounded-3xl border border-line/70 bg-white/90 p-5 shadow-[0_12px_28px_rgba(37,35,27,0.06)]">
            ...
            <p className="mt-2 text-xl font-semibold text-foreground">
              {latestRow?.meteoAmFirstSeenAt
                ? formatMilanDateTimeSeconds(latestRow.meteoAmFirstSeenAt)
                : "—"}
            </p>
            <p className="mt-2 text-sm text-black/65">
              {latestRow?.rawMetar
                ? "First-seen timestamp from the public MeteoAM Deda poll."
                : "No stored MeteoAM first-seen timestamp yet for this date."}
            </p>
          </div>
        </section>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-black/60">
                Stored LIMC METAR rows captured from the public MeteoAM Deda endpoint.
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 h-[420px]">
            {chartData.datasets.length ? (
              ...
              <div className="flex h-full items-center justify-center rounded-2xl border border-dashed border-black/15 bg-black/[0.02] text-sm text-black/55">
                No LIMC MeteoAM observations stored for this date yet.
              </div>
            )}
          </div>
        </section>
        ...
        <section className="rounded-3xl border border-line/80 bg-white/95 p-6 shadow-[0_18px_50px_rgba(37,35,27,0.06)]">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-black/60">
                Recent routine LIMC METAR first-seen timing across the public
                MeteoAM Deda endpoint and NOAA `tgftp`. Times in this table are
                shown in America/Chicago. Both sides are sampled every minute
                because the public Italian release lag is wide and does not fit
                a small watch window.
              </p>
            </div>
          </div>
            ...
            <table className="min-w-full border-collapse text-left text-sm">
              <thead>
                <tr className="border-b border-black/10 text-black/55">
                  ...
                  <th className="px-3 py-2 font-semibold">MeteoAM Seen</th>
                  ...
                  raceRows.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-3 whitespace-nowrap text-black/60">
                        {formatChicagoDateTimeSeconds(row.meteoAmFirstSeenAt)}
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap text-black/60">
                ...
                {rows.length ? (
                  rows.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-3 whitespace-nowrap text-black/60">
                        {row.meteoAmFirstSeenAt
                          ? formatMilanDateTimeSeconds(row.meteoAmFirstSeenAt)
                          : "—"}
                      </td>
                      <td className="px-3 py-3 whitespace-nowrap text-black/60">
                        {row.source}
