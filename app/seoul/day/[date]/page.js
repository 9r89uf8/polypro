...
function WeathercomHourlySummary({ diagnostics, unit, loading }) {
  ...
  const runningPreHour =
    diagnostics?.runningPreHour ??
    (Number.isFinite(live?.biasC) || Number.isFinite(live?.biasF)
      ? live
      : null);
  ...
  const liveStatus = live ? predictionStatusMeta(live.status).label : null;
      ...
      <div className="relative mx-auto flex min-h-screen max-w-[1680px] flex-col px-4 py-5 md:px-8 md:py-7">
        ...
        <section className="flex min-h-0 flex-1 flex-col pt-5">
                ...
                {dayData === undefined
                  ...
                  : refreshState.message ||
                    `${
                      ...
                      latestPrediction?.hourlyCurve?.length ?? 0
                    } ensemble points · ${weathercomHourlyPointCount} Weather.com points`}
              </p>
          ...
          <div id="seoul-weathercom-hourly-description" className="sr-only">
            <p>
              Weather.com hourly forecast revisions and matched AMOS departures
              are summarized above the chart and available in a detailed
              semantic table below it. Signed chart badges mark every hourly
              prediction whose latest distinct value changed by at least one
              degree Celsius. Revision times are when this collector first
              detected the new value, not Weather.com publication times.
            </p>
          </div>
