...
const WEATHERCOM_HOURLY_FORECAST_URL = `${WEATHERCOM_API_BASE_URL}/v3/wx/forecast/hourly/2day`;
...
function normalizeWeatherComHourlyRows(payload, requestedUnit, timeZone) {
  const validTimes = Array.isArray(payload?.validTimeUtc)
    ? payload.validTimeUtc
    : [];
  const temperatures = Array.isArray(payload?.temperature)
    ? payload.temperature
    : [];
  const phrases = Array.isArray(payload?.wxPhraseLong)
    ? payload.wxPhraseLong
    : Array.isArray(payload?.wxPhraseShort)
      ? payload.wxPhraseShort
      : [];
  const cloudCover = Array.isArray(payload?.cloudCover)
    ? payload.cloudCover
    : [];
  const rowCount = Math.max(
    validTimes.length,
  ...
  const rowsByValidTime = new Map();

  for (let index = 0; index < rowCount; index += 1) {
    if (
      validTimes[index] === null ||
      validTimes[index] === undefined ||
      (typeof validTimes[index] === "string" && validTimes[index].trim() === "")
    ) {
      continue;
    }
    const validTimeValue = toFiniteNumber(validTimes[index]);
    if (validTimeValue === null) {
      continue;
    }
    const forecastTimeUtc =
      Math.abs(validTimeValue) < 1_000_000_000_000
        ? validTimeValue * 1000
        : validTimeValue;
    ...
    const phrase = toNonEmptyString(phrases[index]);
    const cloudCoverPct = normalizeCloudCover(cloudCover[index]);
    rowsByValidTime.set(forecastTimeUtc, {
      ...
      ...(phrase ? { phrase } : {}),
      ...(cloudCoverPct !== null ? { cloudCoverPct } : {}),
    });
  }

  return Array.from(rowsByValidTime.values()).sort(
...
export const storeForecastCapture = internalMutationGeneric({
  args: {
    ...
    weathercomHourlyStatus: v.optional(
      v.union(v.literal(WEATHER_STATUS.OK), v.literal(WEATHER_STATUS.ERROR)),
    ),
    ...
    weathercomHourlyRows: v.optional(v.array(hourlyForecastRowValidator)),
...
export const collectForecastSnapshot = internalActionGeneric({
  ...
  handler: async (ctx, args) => {
    ...
    const okCount = [weathercom, weathercomHourly, google, openMeteo].filter(
    ...
    const capture = await ctx.runMutation("seoulWeather:storeForecastCapture", {
      ...
      weathercomHourlyRows: weathercomHourly.rows,
    ...
    return {
      ...
      providerCounts: {
        ...
        weathercomHours: weathercomHourly.rows.length,
